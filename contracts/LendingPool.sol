...
contract LendingPool is ReentrancyGuard, Ownable {
   ...
   struct Institution {
    ...
    function stake(uint256 _amount) external onlyOwner {
        ...
        // Transfer USDE from bank to contract
    ...
    function initiateStakingCooldown() external onlyOwner {
        usde.cooldown();
        emit StakingCooldownInitiated(block.timestamp);
    }


    function distributeToInstitution(uint256 _amount) 
        external 
        onlyOwner 
        onlyDuringDistribution 
    {
        require(_amount <= pool.availableAmount, "Insufficient available amount");
    ...
    function getInstitutionStatus() external view returns (
        uint256 totalBorrowed,
        uint256 totalRepaid,
        uint256 installmentsPaid
    ) {
        return (
            institution.totalBorrowed,
            institution.totalRepaid,
            institution.installmentsPaid
        );
    }
